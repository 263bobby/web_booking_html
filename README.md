...
- [Tóm tắt các bước xây dựng](#tóm-tắt-các-bước-xây-dựng)
...
## � Tóm Tắt Các Bước Xây Dựng

### 🎯 14 Giai Đoạn Phát Triển

#### 1️⃣ Lập Kế Hoạch
- Xác định yêu cầu: Ứng dụng đặt sân thể thao
- Phân tích tính năng: Tìm kiếm, đặt sân, lịch sử, hồ sơ
- Thiết kế wireframe 4 trang chính

#### 2️⃣ Chọn Công Nghệ
- **Stack**: HTML5 + CSS3 + Vanilla JavaScript
- **Tại sao**: Nhẹ, không cần build tools, hiệu suất cao
- **Dữ liệu**: JSON + LocalStorage (client-side only)

#### 3️⃣ Tạo Cấu Trúc Dự Án
```
sporthub/
├── components/    (7 modules tái sử dụng)
├── pages/         (4 HTML pages)
├── scripts/       (3 logic files)
├── styles/        (3 CSS files)
└── data/          (2 JSON files)
```

#### 4️⃣ Chuẩn Bị Dữ Liệu
- **config.json**: Cấu hình (brand, sports, districts, timeSlots)
- **courts.json**: 25 sân với id, name, rating, amenities, price, image

#### 5️⃣ Xây Dựng Components
| Component | Chức Năng |
|-----------|----------|
| Navbar.js | Menu điều hướng |
| Footer.js | Footer |
| CourtCard.js | Thẻ sân |
| Modal.js | Dialog popup |
| Toast.js | Thông báo |
| SportFilter.js | Bộ lọc |
| History.js | Lịch sử đặt |

#### 6️⃣ Hệ Thống Lưu Trữ (store.js)
```javascript
saveBooking(data)         // Lưu đơn đặt hiện tại
getBooking()              // Lấy đơn đặt
saveToHistory(booking)    // Thêm vào lịch sử
getHistory()              // Lấy toàn bộ lịch sử
```

#### 7️⃣ Router & Điều Hướng (router.js)
- `getCurrentPage()` - Lấy trang hiện tại từ data-page
- `goToProfile()` - Điều hướng tới profile page

#### 8️⃣ Xây Dựng 4 Trang HTML
- **home.html**: Hero section + search + featured courts + stats
- **search.html**: Filters + court grid + responsive layout
- **booking.html**: Court info + date picker + time slots + form
- **profile.html**: Stats + booking history + search/filter

#### 9️⃣ Logic Chính (main.js)
```javascript
loadData()           // Fetch config.json & courts.json
initHome()           // Render & setup home page
initSearch()         // Render search & filter logic
initBooking()        // Handle booking form submission
initProfile()        // Render profile & history
```

#### 🔟 Responsive Design
- **Breakpoints**: 768px (tablet), 1024px (desktop)
- **CSS Architecture**: variables.css + main.css + components.css
- **Công nghệ**: CSS Grid, Flexbox, Media queries

#### 1️⃣1️⃣ Tính Năng Chính
- ✅ Tìm kiếm: Filter array courts theo keyword
- ✅ Lọc: Sport, district, price range
- ✅ Đặt sân: Form validation + localStorage
- ✅ Lịch sử: getHistory() + render table
- ✅ Thống kê: Sum total bookings & spending
...
#### 1️⃣4️⃣ Triển Khai
```bash
git add README.md sporthub/data/courts.json
git commit -m "docs: add README and update courts to c25"
...
|------|----------|--------|
| 1 | Lập kế hoạch | Wireframe, yêu cầu |
| 2 | Chọn tech | HTML5+CSS3+JS Vanilla |
| 3 | Cấu trúc | Folder hierarchy |
| 4 | Dữ liệu | 2 JSON files (config + 25 courts) |
| 5 | Components | 7 reusable modules |
| 6 | Store | localStorage API |
| 7 | Router | Page navigation |
| 8 | Pages | 4 HTML pages |
| 9 | Logic | main.js với 4 hàm init |
| 10 | CSS | Responsive design |
| 11 | Features | Search, filter, booking, history |
| 12 | UX | Validation, notifications, smooth animations |
| 13 | Docs | README + comments |
| 14 | Deploy | Git push & ready for hosting |

### 🚀 Kết Quả Cuối Cùng
- ✅ Ứng dụng web hoạt động 100%
